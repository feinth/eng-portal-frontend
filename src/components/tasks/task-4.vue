<template>
  ...
  <div
    v-if="prepareStarted || recordStarted"
  ...
  >
    <div>
      ...
      <div class="image-container text-gray-800 mt-4">
        <q-img :src="imgSrc + task.images[0].image" class="image-max-size" />
        <q-img :src="imgSrc + task.images[1].image" class="image-max-size" />
      </div>
    </div>
  </div>
...
.image-container {
  ...
  gap: 120px; /* Отступ между изображениями */
}
...
.image-text {
  position: absolute;
  top: 50%; /* Расположить по середине изображения */
  left: 50%; /* Центрировать по горизонтали */
  transform: translate(
    -50%,
    -50%
  ); /* Сдвинуть текст для точного центрирования */
  color: white; /* Цвет текста (или другой, если нужно) */
  font-weight: bold; /* Жирный шрифт для текста */
  text-shadow: 2px 2px 4px rgba(0, 0, 0, 0.6); /* Тень для улучшения читабельности */
  text-align: center;
}
</style>
