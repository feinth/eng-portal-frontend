<template>
  <div class="p-4">
    <div v-if="currentTaskIndex < tasks.length">
      <TaskPrepareComponent :task="tasks[currentTaskIndex]" @prepare-finished="handlePrepareFinished" :key="currentTaskIndex" v-if="!prepareComplete"/> 
      <TaskAnswerComponent :task="tasks[currentTaskIndex]" @task-finished="handleTaskFinished" :key="currentTaskIndex" v-if="prepareComplete"/>
    </div>
  </div>
</template>
      ...
      tasks: [
        {
          ...
          taskHeader: 'You are going to read the text aloud. You have 1.5 minutes to read text silently, and then be ready to read it aloud. Remember that you will not have more than 2 minutes for reading aloud.',
          taskNumber: 1
        },
        {
          prepareTimeout: 45,
          recordTimeout: 90,
          taskText: "It is a long established fact that a reader will be distracted by the readable content of a page when looking at its layout. The point of using Lorem Ipsum is that it has a more-or-less normal distribution of letters, as opposed to using 'Content here, content here', making it look like readable English. Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for 'lorem ipsum' will uncover many web sites still in their infancy. Various versions have evolved over the years, sometimes by accident, sometimes on purpose (injected humour and the like).",
